...
path = os.getcwd()
ell = np.loadtxt(path+"/ell_array_120.txt", usecols=(0))
r = np.loadtxt(path+"/angularsep_arcmins_20bins.txt", usecols=(0))
r_min = np.loadtxt(path+"/angularsep_arcmins_min_20bins.txt", usecols=(0))
r_max = np.loadtxt(path+"/angularsep_arcmins_max_20bins.txt", usecols=(0))
...
def log_prior(theta):
    ...
    if 0.06 <= Mv <= 0.62 and 0.18 <= Omega_m <= 0.42 and 1.29e-9 < As < 2.91e-9:
        return 0.0
    return -np.inf
...
def log_likelihood(theta, data, cov):
    ...
    B = np.array([[  213622.94912239,   120722.43499073,   237355.95359837,
            7930.84688141,   -77519.13174316,   528938.90293084,
         -310959.20004962,     4363.23531266,   384997.68624109,
          296252.35907312,  -133477.41740964,  -196955.95804689,
          375785.68308481,   129530.60251565,   166076.9977687 ,
          -42030.2595881 ,   392952.52292308,   173978.60183342,
          310337.51740295,   160559.95767032],
       [  -41533.00160003,    50909.9489008 ,  -108078.83737227,
         -313151.80214371,  -138044.05836699,  -293046.67821704,
          184506.70678012,  -524011.44134617,   179117.26502887,
         -445530.07666196,  -116657.16629004,   201482.06323276,
          -47988.50714078,  -136098.28951282,   586112.23770701,
         -253677.6926785 ,   547791.18350243,   320239.6744688 ,
          555703.94871788,  1347509.03253187],
       [-1423367.15965279,  -378156.57614255,   270108.82467619,
          822146.04476156,   923207.43877641,  1037148.8477299 ,
          650118.03706263,   513665.95235083,  -343352.07267599,
          250053.19825488,  -861360.99853308,    35704.31878435,
         -541807.10327392,  -773461.39365722,  -695095.75714191,
         -437212.33059669,  -234055.13181125,   -33266.28135565,
         -204846.69654008,  1418326.8148534 ]])
    compressed_diff = np.dot(B, data) - np.dot(B, model)
    compressed_cov = np.dot(np.dot(B, cov), B.T)
    gaussian_likelihood = -0.5 * np.dot(compressed_diff, hartlap_factor*np.linalg.solve(compressed_cov, compressed_diff))
    #gaussian_likelihood = -0.5 * np.dot(compressed_diff.T, np.dot(compressed_cov_inverse, compressed_diff))
...
path = os.getcwd()
data_vec = pd.read_csv(path+'/massivenus_2pt_z1.5_noiseless_1e4realizations.csv', sep=' ')
'''
...
corr2pt = pd.read_csv(path+'/massivenus_2pt_z1.5_noisy_1e4realizations.csv', sep=' ')
'''
...
np.random.seed(42)
theta1_0 = np.random.uniform(0.06, 0.62, size=walkers) #Mv
theta2_0 = np.random.uniform(0.18, 0.42, size=walkers) #Omega_m
theta3_0 = np.random.uniform(1.29e-9, 2.91e-9, size=walkers) #A_s
...
filename = "MCMC_%swalkers_z%s_LSST_binave_compressed.h5"%(nwalkers, z_source)
